  ...
  return (
    <div className="min-h-screen">
      <section className="relative h-[80vh] overflow-hidden">
        ...
        <button
          onClick={prevImage}
          className="absolute left-4 top-1/2 transform -translate-y-1/2 z-20 bg-primary/60 text-white p-3 rounded-full hover:bg-black/70 transition-all"
        ...
        <button
          onClick={nextImage}
          className="absolute right-4 top-1/2 transform -translate-y-1/2 z-20 bg-primary/60 text-white p-3 rounded-full hover:bg-black/70 transition-all"
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center px-4">
          <div className="mx-auto p-5 rounded">
            <motion.h1
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="text-4xl md:text-6xl font-bold text-base-content text-center mb-6"
            >
            ...
            <motion.p
              ...
              className="text-xl text-base-content text-center mb-8"
            >
        ...
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-20 flex space-x-2">
          {backgroundImages.map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${currentImageIndex === index ? 'bg-primary w-6' : 'bg-primary/50'
                }`}
            />
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {featuredProperties.map((property) => (
              <motion.div
                ...
                className="bg-base-100 hover:bg-base-200 rounded-lg overflow-hidden shadow-lg"
              >
                <img src={property.image} alt={property.title} className="w-full h-48 object-cover" />
                <div className="p-6">
                  <h3 className="text-xl text-primary font-semibold mb-2">{property.title}</h3>
                  <p className="text-base-content mb-2">{property.location}</p>
                  <p className="text-base text-base-content font-bold">{property.price}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            >
              ...
              <p className="text-base-content">Browse through thousands of properties that match your criteria</p>
            </motion.div>
            <motion.div
              whileHover={{ y: -10 }}
              className="text-center p-6"
            >
              ...
              <h3 className="text-xl font-semibold mb-3 text-primary">Best Prices</h3>
              <p className="text-base-content">Find the best deals and competitive prices in the market</p>
            </motion.div>
            <motion.div
              whileHover={{ y: -10 }}
              className="text-center p-6"
            >
              ...
              <h3 className="text-xl font-semibold mb-3 text-primary">Trusted Agents</h3>
              <p className="text-base-content">Work with verified and professional real estate agents</p>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-4 bg-base-200">
        <div className="max-w-4xl mx-auto text-center text-white">
          ...
          <Link to="/properties" className="btn btn-primary btn-wide rounded-full font-semibold">
            Get Started
          </Link>
        </div>
      </section>
